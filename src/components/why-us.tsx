...
import { ScanSearch, BadgeEuro, MapPinned, Award } from "lucide-react";
      ...
      <div className="mx-auto max-w-[1400px] px-5 lg:px-10 py-20 lg:py-28">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5 lg:sticky lg:top-28 lg:self-start">
            <SectionLabel index="06" title="Pourquoi G.M.S.S" />
            <h2 className="display-lg mt-5 text-navy">
              Le sérieux d&apos;un grossiste,{" "}
              <span className="text-blue-600">la souplesse</span> d&apos;une
              structure courte.
            </h2>
            <p className="mt-7 text-steel-700 leading-relaxed max-w-md">
              {`G.M.S.S est une micro-entreprise — c'est un avantage : décisions rapides, marges transparentes, un seul interlocuteur du devis à la livraison.`}
            </p>

            {/* Sourcing image */}
            <div className="mt-9 relative aspect-[4/3] overflow-hidden border border-rule">
              <Image
                src={img("briefDesk", 900, 75)}
                alt={images.briefDesk.alt}
                fill
          ...
          <ul className="lg:col-span-7 lg:col-start-7 space-y-5">
            {principles.map((p, i) => {
              const Icon = icons[p.label] ?? ScanSearch;
              return (
